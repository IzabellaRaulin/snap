...
func (p *pluginControl) ExpandWildCards(ns []string) ([][]string) {
	...
	// support wildcards in metric namespace
	fmt.Fprintf(os.Stderr, "Debug in control.go, ExpandWildCards\n")

	//TODO zobaczyc jak z obsługa błedu
...
func (p *pluginControl) validateMetricTypeSubscription(mt core.RequestedMetric, cd *cdata.ConfigDataNode) (core.Metric, []serror.SnapError) {
	...
	for i := range ms {
		...
		typ, serr := core.ToPluginType(ms[i].Plugin.TypeName())
		if serr != nil {
...
func (p *pluginControl) GetMetric(ns []string, ver int) (core.CatalogedMetric, error) {
	fmt.Fprintf(os.Stderr, "Debug: !!!!!!!!!!!! Get METRIC jest wolane\n\n")
	m, err := p.metricCatalog.GetMetric(ns, ver)
	return m, err
}
...
func (p *pluginControl) CollectMetrics(metricTypes []core.Metric, deadline time.Time, taskID string) (metrics []core.Metric, errs []error) {
	fmt.Fprintf(os.Stderr, "Debug, Expand in Collect Metrics\n")

	//expanded metric types (based on wildcard rule)
	...
	for _, m := range metricTypes {
		...
		for _, ns := range nss {
			fmt.Fprintf(os.Stderr, "Debug in Collect Metrics, name=%+v\n", ns)
			//namespace []string, timestamp time.Time, source string, tags map[string]string, labels []core.Label, data interface{}) *PluginMetricType
		...
		// fmt.Fprintf(os.Stderr, "Debug: CM; metricTypes=%+v", m.Namespace())
	}
