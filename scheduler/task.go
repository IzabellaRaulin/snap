...
func (t *task) Spin() {
	...
	t.Lock()
	defer t.Unlock()
	// Reset the lastFireTime at each Spin to time.Time{}
	// This ensures that for the first run
	// - for simple schedule, an execution takes place immediately
	// - for windowed schedule, an execution takes place immediately on window start time

	// Also, this ensures that misses are tracked only forward of the point
	...
	if t.state == core.TaskStopped || t.state == core.TaskCompleted {
...
func (t *taskCollection) remove(task *task) error {
	...
	defer t.Unlock()
	if _, ok := t.table[task.id]; ok {
		if task.state != core.TaskStopped && task.state != core.TaskDisabled && task.state != core.TaskCompleted {
			taskLogger.WithFields(log.Fields{
				"_block":  "remove",
				"task id": task.id,
