...
func TestStartTask(t *testing.T) {
	...
	Convey("Calling StartTask on an ended task", t, func() {
		start := time.Now().Add(startWait)
		stop := time.Now().Add(startWait + windowSize)
		...
		// try to restart the ended task
		err := s.StartTask(tsk.ID())
		Convey("Should return an error", func() {
			So(err, ShouldNotBeNil)
		})
		Convey("Error should read: Task is ended. Cannot be restarted.", func() {
			So(err[0].Error(), ShouldEqual, ErrTaskEndedNotRunnable.Error())
		})
		Convey("State of the task should be still TaskEnded", func() {
			So(tsk.State(), ShouldEqual, core.TaskEnded)
		})
